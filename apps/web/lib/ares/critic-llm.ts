...
// We do not actually call Claude Haiku — the proxy translates this to the
// small-model row in each provider's mapping (see apps/api/src/routes/anthropic.ts).
...
export async function critic(opts: CriticOpts): Promise<string> {
  ...
  if (opts.matterId) body.matter_id = opts.matterId;

  const res = await anthropicFetch(body);
  if (!res.ok) throw new Error(`ARES critic call failed: HTTP ${res.status}`);
  const json = (await res.json()) as AnthropicLikeResponse;
