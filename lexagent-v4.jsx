...
// vaultStore — shared:true means keys persist across devices for the same user
const vaultStore = {
...
// ── Theme ──────────────────────────────────────────────────────────────────
const T = {
  bg:"#060810",bg2:"#080B14",surface:"#0C1020",panel:"#101525",panel2:"#141929",
  border:"#1A2035",borderHi:"#232A42",
  gold:"#C8A96E",goldBright:"#DEC080",goldDim:"#7A6535",goldFaint:"#C8A96E12",
  platinum:"#A8B4CC",platDim:"#5A6880",
  text:"#D0D6E8",textSub:"#62728E",textMuted:"#2E364A",
  crimson:"#C43355",crimsonFaint:"#C4335518",
  cobalt:"#3A6FD8",cobaltFaint:"#3A6FD818",
  emerald:"#28A868",emeraldFaint:"#28A86818",
  amber:"#D08030",amberFaint:"#D0803018",
  violet:"#8A55D8",violetFaint:"#8A55D818",
  verified:"#28A868",hallucinated:"#C43355",unverified:"#D08030",
};
...
async function courtListenerSearchDirect(query, jurisdiction, token) {
  ...
  const url = `https://www.courtlistener.com/api/rest/v4/search/?q=${encodeURIComponent(query)}&type=o&semantic=true&order_by=score+desc${jParam}`;
...
async function courtListenerJudgeLookup(judgeName, token) {
  const headers = { "Accept": "application/json" };
  if (token) headers["Authorization"] = `Token ${token}`;
  const url = `https://www.courtlistener.com/api/rest/v4/people/?name__icontains=${encodeURIComponent(judgeName)}&type=jud&fields=id,name_full,date_dob,date_dod,gender,race,political_affiliations,positions,educations,aba_ratings,sources`;
  const res = await fetch(url, { headers });
  ...
  const posUrl = `https://www.courtlistener.com/api/rest/v4/positions/?person=${j.id}&fields=court,position_type,date_start,date_termination,appointer,how_selected,termination_reason`;
  let positions = [];
  try {
...
async function harvardCAPSearch(query, jurisdiction) {
  const jParam = jurisdiction && !["Other State","Federal – SCOTUS","Federal – Circuit","Federal – District"].includes(jurisdiction)
    ...
    : "";
  const url = `https://api.case.law/v1/cases/?search=${encodeURIComponent(query)}&full_case=false&page_size=5${jParam}`;
...
async function govInfoStatuteLookup(query, govInfoKey) {
  ...
  const url = `https://api.govinfo.gov/search?query=${encodeURIComponent(query)}&pageSize=5&offsetMark=*&collections=USCODE,CFR,FR&api_key=${key}`;
  const res = await fetch(url, { headers: { "Accept": "application/json" } });
...
body{background:${T.bg};color:${T.text};font-family:'Plus Jakarta Sans',sans-serif;font-size:14px;line-height:1.5;-webkit-font-smoothing:antialiased}
::-webkit-scrollbar{width:3px;height:3px}::-webkit-scrollbar-track{background:transparent}::-webkit-scrollbar-thumb{background:${T.border};border-radius:99px}
...
@keyframes shieldPulse{0%,100%{box-shadow:0 0 0 0 ${T.emerald}00}50%{box-shadow:0 0 0 6px ${T.emerald}00}}
@keyframes slideModal{from{opacity:0;transform:translateY(30px)}to{opacity:1;transform:translateY(0)}}
@keyframes sheetUp{from{transform:translateY(100%)}to{transform:translateY(0)}}
@keyframes verifyPop{0%{transform:scale(0.8);opacity:0}100%{transform:scale(1);opacity:1}}
...
.gold-focus:focus{border-color:${T.gold}60 !important;box-shadow:0 0 0 3px ${T.gold}10}
.interactive{transition:all 0.15s;cursor:pointer}
.interactive:hover{border-color:${T.borderHi} !important}
.case-card{transition:all 0.15s;cursor:pointer}
.case-card:hover{border-color:${T.borderHi};background:${T.panel}}
...
.cite-verified{background:${T.emerald}22;color:${T.emerald};border:1px solid ${T.emerald}44;border-radius:3px;padding:1px 6px;font-size:10px;font-family:'JetBrains Mono',monospace;letter-spacing:0.04em}
.cite-unconfirmed{background:${T.amber}22;color:${T.amber};border:1px solid ${T.amber}44;border-radius:3px;padding:1px 6px;font-size:10px;font-family:'JetBrains Mono',monospace;letter-spacing:0.04em}
.conf-high{background:${T.emerald}22;color:${T.emerald};border-radius:3px;padding:1px 5px;font-size:10px;font-family:'JetBrains Mono',monospace}
.conf-med{background:${T.amber}22;color:${T.amber};border-radius:3px;padding:1px 5px;font-size:10px;font-family:'JetBrains Mono',monospace}
.conf-low{background:${T.crimson}22;color:${T.crimson};border-radius:3px;padding:1px 5px;font-size:10px;font-family:'JetBrains Mono',monospace}

/* Shield badge */
.shield-active{background:${T.emerald}18;border-color:${T.emerald}50 !important}
.shield-warning{background:${T.amber}18;border-color:${T.amber}50 !important}
.shield-danger{background:${T.crimson}18;border-color:${T.crimson}50 !important}
        ...
        <div className="scroll-y" style={{flex:1,padding:"14px 14px 6px",display:"flex",flexDirection:"column",gap:12}}>
          {msgs.map(m=>(
            <div key={m.id} style={{display:"flex",gap:9,animation:"fadeUp 0.2s ease both"}}>
              <div style={{width:28,height:28,borderRadius:7,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",background:m.role==="user"?`${T.gold}22`:T.panel,border:`1px solid ${m.role==="user"?T.goldDim:T.border}`,marginTop:2}}>
                <span style={{fontSize:9,color:m.role==="user"?T.gold:T.platinum,fontFamily:"'JetBrains Mono',monospace"}}>{m.role==="user"?"YOU":"AI"}</span>
              </div>
              <div style={{flex:1,minWidth:0}}>
          ...
          {loading&&(
            <div style={{display:"flex",gap:9}}>
              <div style={{width:28,height:28,borderRadius:7,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",background:T.panel,border:`1px solid ${T.border}`}}><Spinner size={12}/></div>
              <div style={{background:T.panel,border:`1px solid ${T.border}`,borderRadius:7,padding:"10px 14px",display:"flex",gap:6,alignItems:"center"}}>
                <span style={{fontSize:12,color:T.textSub}}>Researching</span>
                {[0,1,2].map(i=><div key={i} style={{width:4,height:4,borderRadius:"50%",background:T.gold,animation:`blink 1.2s ${i*0.2}s infinite`}}/>)}
              </div>
            </div>
          )}
          <div ref={endRef}/>
            ...
            <div className="tab-scroll" style={{display:"flex",gap:4,flex:1}}>
              {QUICK.map((q,i)=>(
                <button key={i} onClick={()=>{setInput(q);setTimeout(()=>inputRef.current?.focus(),0);}}
                  style={{fontSize:10,color:T.textMuted,background:T.panel2,border:`1px solid ${T.border}`,borderRadius:4,padding:"4px 8px",cursor:"pointer",whiteSpace:"nowrap",transition:"all 0.12s",fontFamily:"'Plus Jakarta Sans',sans-serif"}}
...
function DraftPanel({caseData,settings,onLog,isMobile,notify}) {
  const DTYPES = [
    {id:"memo",label:"Research Memo"},{id:"motion_dismiss",label:"Motion to Dismiss"},
    {id:"summary_judgment",label:"Summary Judgment"},{id:"demand",label:"Demand Letter"},
    {id:"opening",label:"Opening Statement"},{id:"cross_exam",label:"Cross-Examination Qs"},
    {id:"settlement",label:"Settlement Memo"},{id:"appeal",label:"Notice of Appeal"},
  ];
      ...
      try {
        ...
        const snippets = await Promise.all(
          verifiedCites.slice(0,3).map(async v => {
            try {
              const searchUrl = `https://www.courtlistener.com/api/rest/v4/search/?q=${encodeURIComponent(v.citation)}&type=o&fields=caseName,citation,court_id,dateFiled,headmatter&page_size=1`;
    ...
    <div style={{display:"flex",flexDirection:isMobile?"column":"row",gap:12,height:"100%",overflow:"hidden"}}>
        ...
        <Panel style={{padding:14}}>
          ...
          {DTYPES.map(dt=>(
            <div key={dt.id} onClick={()=>{ setType(dt.id); if(isMobile) setMobileView("output"); }}
              ...
              <span style={{fontSize:12,color:type===dt.id?T.gold:T.textSub}}>{dt.label}</span>
              {isMobile&&<Icon n="arrow" size={12} color={T.textMuted} style={{marginLeft:"auto"}}/>}
            </div>
          ))}
        </Panel>
        <Panel style={{padding:14}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",background:T.surface,border:`1px solid ${T.border}`,borderRadius:8,overflow:"hidden"}}>
        <div style={{padding:"9px 14px",borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
          ...
          {draft&&(
            <div style={{display:"flex",gap:5}}>
              <Btn variant="cobalt" size="xs" icon="pdf" onClick={()=>doExport("pdf")} disabled={!!exporting}>PDF</Btn>
              <Btn variant="emerald" size="xs" icon="word" onClick={()=>doExport("docx")} disabled={!!exporting}>Word</Btn>
        ...
        <div className="fade-up">
          <Panel style={{padding:18,marginBottom:12}}>
            <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
              <div>
                <div className="serif" style={{fontSize:22,color:T.text,marginBottom:2}}>{intel.name}</div>
                <div style={{fontSize:12,color:T.textSub,marginBottom:8}}>{intel.court}</div>
                <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                  ...
                  {intel.dataSource&&<Badge color={intel.dataSource.includes("CourtListener")?T.cobalt:T.textSub} size="xs">{intel.dataSource.includes("CourtListener")?"🏛 Real DB":"Web Search"}</Badge>}
                  {intel.politicalAffiliation&&<Badge color={T.textSub} size="xs">{intel.politicalAffiliation}</Badge>}
                  {intel.abaRating&&<Badge color={T.amber} size="xs">ABA: {intel.abaRating}</Badge>}
                </div>
              </div>
              <div style={{display:"flex",gap:16,flexShrink:0}}>
                <div style={{textAlign:"center"}}>
                  <div style={{fontSize:9,color:T.crimson,fontFamily:"'JetBrains Mono',monospace",letterSpacing:"0.06em",marginBottom:4}}>PRO WIN %</div>
                  <div style={{fontSize:24,fontWeight:700,color:T.crimson,fontFamily:"'JetBrains Mono',monospace"}}>{intel.rulingTendencies?.prosecution??"—"}</div>
                </div>
                <div style={{textAlign:"center"}}>
                  <div style={{fontSize:9,color:T.cobalt,fontFamily:"'JetBrains Mono',monospace",letterSpacing:"0.06em",marginBottom:4}}>DEF WIN %</div>
                  <div style={{fontSize:24,fontWeight:700,color:T.cobalt,fontFamily:"'JetBrains Mono',monospace"}}>{intel.rulingTendencies?.defense??"—"}</div>
                </div>
              </div>
            </div>
            {intel.rulingTendencies?.notes&&<div style={{marginTop:12,fontSize:12,color:T.textSub,lineHeight:1.65,borderTop:`1px solid ${T.border}`,paddingTop:10}}>{intel.rulingTendencies.notes}</div>}
          </Panel>

          <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:12,marginBottom:12}}>
            <Panel style={{padding:16}}>
              <SectionHeader label="Winning Strategies" color={T.emerald}/>
              {(intel.winningStrategies||[]).map((s,i)=><div key={i} style={{display:"flex",gap:7,marginBottom:8}}><span style={{color:T.emerald,flexShrink:0}}>✓</span><span style={{fontSize:12,color:T.text,lineHeight:1.6}}>{s}</span></div>)}
            </Panel>
            <Panel style={{padding:16}}>
              <SectionHeader label="Red Flags — Avoid" color={T.crimson}/>
              {(intel.redFlags||[]).map((r,i)=><div key={i} style={{display:"flex",gap:7,marginBottom:8}}><span style={{color:T.crimson,flexShrink:0}}>⚠</span><span style={{fontSize:12,color:T.text,lineHeight:1.6}}>{r}</span></div>)}
            </Panel>
            <Panel style={{padding:16}}>
...
function Dashboard({cases,onSelect,onNew,logs,isMobile}) {
  const stats = [
    {l:"Active",v:cases.filter(c=>c.status==="Active").length,c:T.emerald},
    {l:"Precedents",v:cases.reduce((a,c)=>a+(c.precedents?.length||0),0),c:T.gold},
    {l:"Citations Verified",v:cases.reduce((a,c)=>a+(c.allVerifications||[]).filter(v=>v.status==="verified").length,0),c:T.emerald},
    {l:"Shield Alerts",v:cases.reduce((a,c)=>a+(c.allVerifications||[]).filter(v=>v.status==="not_found").length,0),c:T.crimson},
  ];
  ...
  return (
    <div className="scroll-y" style={{height:"100%"}}>
      <div style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between",marginBottom:20,gap:12,flexWrap:"wrap"}}>
        <div>
          <div className="serif" style={{fontSize:isMobile?24:30,fontStyle:"italic",color:T.text,lineHeight:1.1}}>Case Intelligence</div>
          <div style={{fontSize:12,color:T.textSub,marginTop:5}}>{cases.length} matters · {new Date().toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric"})}</div>
        </div>
        <Btn onClick={onNew} icon="plus">Open Matter</Btn>
      </div>
      <div style={{display:"grid",gridTemplateColumns:`repeat(${isMobile?2:4},1fr)`,gap:10,marginBottom:20}}>
        {stats.map(s=>(
          <Panel key={s.l} style={{padding:"14px 16px"}}>
            <div style={{fontSize:9,color:T.textMuted,fontFamily:"'JetBrains Mono',monospace",letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:8}}>{s.l}</div>
      ...
      {!cases.length?(
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"60px 20px",gap:14,textAlign:"center"}}>
          <div style={{width:60,height:60,borderRadius:14,background:T.goldFaint,border:`1px solid ${T.goldDim}`,display:"flex",alignItems:"center",justifyContent:"center"}}><Icon n="scale" size={26} color={T.gold}/></div>
          <div className="serif" style={{fontSize:22,color:T.textSub,fontStyle:"italic"}}>No matters open</div>
          <Btn onClick={onNew} icon="plus" size="lg">Open First Matter</Btn>
        </div>
      ):(
        <div style={{display:"grid",gridTemplateColumns:`repeat(auto-fill,minmax(${isMobile?"100%":"300px"},1fr))`,gap:10}}>
          {cases.map(c=>{
            ...
            return (
              <div key={c.id} onClick={()=>onSelect(c)} className="case-card" style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:8,padding:16,cursor:"pointer"}}>
                <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:8,marginBottom:7}}>
                  <div className="serif" style={{fontSize:15,color:T.text,lineHeight:1.35,flex:1}}>{c.title}</div>
                  <Badge color={STATUS_C[c.status]||T.textSub} dot size="xs">{c.status}</Badge>
                  {c._shared&&<Badge color={T.violet} size="xs">Team</Badge>}
                </div>
                <div style={{fontSize:12,color:T.textSub,marginBottom:8}}>{c.client}</div>
                <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:8}}>
                  <Badge color={T.cobalt} size="xs">{c.caseType}</Badge>
                  <Badge color={T.textMuted} size="xs">{c.jurisdiction.split("–")[0].trim()}</Badge>
                </div>
                {c.facts&&<div style={{fontSize:11,color:T.textMuted,lineHeight:1.55,marginBottom:8,overflow:"hidden",display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical"}}>{c.facts}</div>}
                <Divider my={6}/>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
                    {c.precedents?.length>0&&<Badge color={T.gold} size="xs">{c.precedents.length} prec.</Badge>}
                    {vAll.length>0&&<Badge color={hasBad?T.crimson:T.emerald} size="xs">{hasBad?`⚠ ${vAll.filter(v=>v.status==="not_found").length} unverified`:`✓ ${vAll.filter(v=>v.status==="verified").length} verified`}</Badge>}
...
export default function LexAgent() {
      ...
      try{
        ...
        const sharedCases = await Promise.all(
          sharedIds.map(async id=>{
            try{
              const raw = await window.storage.get(`lex4-shared-case-${id}`,true);
  ...
  const createCase = async form=>{
    ...
    if(form.shared){
      try{
        ...
        await window.storage.set(`lex4-shared-case-${nc.id}`,JSON.stringify(nc),true);
  ...
  if(!loaded) return (
    <div style={{background:T.bg,height:"100vh",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:16}}>
      <div className="serif" style={{fontSize:28,color:T.gold,fontStyle:"italic"}}>LexAgent</div>
      <Spinner size={22}/>
      <div style={{fontSize:11,color:T.textMuted,fontFamily:"'JetBrains Mono',monospace"}}>ARES INITIALIZING · HALLUCINATION SHIELD LOADING…</div>
    </div>
  );
      ...
      <div style={{display:"flex",height:"100vh",background:T.bg,overflow:"hidden",flexDirection:"column"}}>
        {!isMobile?(
          <div style={{display:"flex",flex:1,overflow:"hidden"}}>
            {/* Sidebar */}
            <div className="desktop-sidebar" style={{width:isTablet?54:218,background:T.bg2,borderRight:`1px solid ${T.border}`,display:"flex",flexDirection:"column",flexShrink:0}}>
              <div style={{padding:isTablet?"14px 0":"18px 16px 14px",borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",justifyContent:isTablet?"center":"flex-start",gap:10}}>
                <div style={{width:28,height:28,borderRadius:7,background:T.goldFaint,border:`1px solid ${T.goldDim}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                  <Icon n="scale" size={14} color={T.gold}/>
                </div>
                {!isTablet&&<div>
                  <div className="serif" style={{fontSize:17,color:T.gold,fontStyle:"italic",lineHeight:1}}>LexAgent</div>
                  <div style={{fontSize:8,color:T.textMuted,fontFamily:"'JetBrains Mono',monospace",letterSpacing:"0.1em",marginTop:2}}>HALLUCINATION SHIELD ON</div>
                </div>}
              </div>

              {!isTablet&&(
                <div onClick={()=>setShowCmd(true)} style={{margin:"10px 8px 4px",display:"flex",alignItems:"center",gap:7,background:T.panel,border:`1px solid ${T.border}`,borderRadius:6,padding:"7px 10px",cursor:"pointer",transition:"border-color 0.15s"}}
                  onMouseEnter={e=>e.currentTarget.style.borderColor=T.borderHi} onMouseLeave={e=>e.currentTarget.style.borderColor=T.border}>
                  <Icon n="search" size={12} color={T.textMuted}/><span style={{fontSize:11,color:T.textMuted,flex:1}}>Search…</span><Badge color={T.textMuted} size="xs">⌘K</Badge>
                </div>
              )}

              <div style={{padding:isTablet?"8px 6px":"6px 8px"}}>
                {[{id:"dashboard",label:"All Matters",icon:"home"},{id:"admin",label:"Admin",icon:"admin"}].map(n=>{
                  const active = (view===n.id&&!inCase);
                  return (
                    <div key={n.id} onClick={()=>{setView(n.id);setSel(null);}} title={isTablet?n.label:undefined}
                      style={{display:"flex",alignItems:"center",gap:8,padding:isTablet?"9px":"7px 10px",borderRadius:6,cursor:"pointer",marginBottom:2,background:active?T.panel:"transparent",borderLeft:isTablet?"none":`2px solid ${active?T.gold:"transparent"}`,transition:"all 0.12s",justifyContent:isTablet?"center":"flex-start"}}>
                      <Icon n={n.icon} size={14} color={active?T.gold:T.textSub}/>
                      {!isTablet&&<span style={{fontSize:12,color:active?T.text:T.textSub,fontWeight:active?500:400}}>{n.label}</span>}
                    </div>
                  );
                })}
              </div>

              {!isTablet&&<Divider my={0}/>}

              {!isTablet&&(
                <div style={{flex:1,overflowY:"auto",padding:"4px 8px"}}>
                  <div style={{fontSize:9,color:T.textMuted,fontFamily:"'JetBrains Mono',monospace",letterSpacing:"0.08em",padding:"4px 10px 8px",textTransform:"uppercase"}}>Open Matters</div>
                  {cases.map(c=>{
                    const active = sel?.id===c.id;
                    const hasBad = (c.allVerifications||[]).some(v=>v.status==="not_found");
                    return (
                      <div key={c.id} onClick={()=>openCase(c)} style={{padding:"7px 10px",borderRadius:5,cursor:"pointer",marginBottom:2,background:active?T.panel:"transparent",borderLeft:`2px solid ${active?T.gold:"transparent"}`,transition:"all 0.1s"}}>
                  ...
                  <div onClick={()=>setShowNew(true)} style={{display:"flex",alignItems:"center",gap:6,padding:"7px 10px",borderRadius:5,cursor:"pointer",marginTop:6,border:`1px dashed ${T.border}`,transition:"border-color 0.15s"}}
                    onMouseEnter={e=>e.currentTarget.style.borderColor=T.goldDim} onMouseLeave={e=>e.currentTarget.style.borderColor=T.border}>
                    <Icon n="plus" size={11} color={T.textMuted}/><span style={{fontSize:11,color:T.textMuted}}>New Matter</span>
                  </div>
                </div>
              )}
            ...
            <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
              {inCase&&(
                <div style={{background:T.bg2,borderBottom:`1px solid ${T.border}`,padding:"0 16px",display:"flex",alignItems:"center",justifyContent:"space-between",height:50,flexShrink:0,gap:12}}>
                  <div style={{display:"flex",alignItems:"center",gap:10,minWidth:0,flex:1}}>
                    <div style={{minWidth:0}}>
                      <div className="serif truncate" style={{fontSize:15,color:T.text,fontWeight:500}}>{sel.title}</div>
                      <div style={{fontSize:10,color:T.textMuted,fontFamily:"'JetBrains Mono',monospace",marginTop:1}}>
                        {sel.client} · {sel.jurisdiction.split("–")[0].trim()}
                        {(sel.allVerifications||[]).length>0&&(
                          <span style={{marginLeft:8,color:(sel.allVerifications||[]).some(v=>v.status==="not_found")?T.crimson:T.emerald}}>
                            · Shield: {(sel.allVerifications||[]).filter(v=>v.status==="verified").length}/{(sel.allVerifications||[]).length} ✓
                          </span>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="tab-scroll" style={{display:"flex",gap:2,alignItems:"center"}}>
                    {TABS.map(t=>{
                      ...
                      return (
                        <button key={t.id} onClick={()=>setTab(t.id)}
                          style={{display:"flex",alignItems:"center",gap:5,padding:"5px 9px",background:active?T.panel:"transparent",border:`1px solid ${active?T.border:"transparent"}`,borderRadius:5,cursor:"pointer",color:active?T.text:hasBad?T.crimson:T.textSub,fontSize:11,fontWeight:active?500:400,transition:"all 0.12s",fontFamily:"'Plus Jakarta Sans',sans-serif",whiteSpace:"nowrap",position:"relative"}}>
                          <Icon n={t.icon} size={12} color={active?T.gold:hasBad?T.crimson:T.textSub}/>{t.label}
                          {hasBad&&<span style={{width:5,height:5,borderRadius:"50%",background:T.crimson,position:"absolute",top:3,right:3}}/>}
                        </button>
                      );
                    })}
                    <div style={{width:1,height:18,background:T.border,margin:"0 4px"}}/>
                    <Btn variant="danger" size="sm" onClick={()=>deleteCase(sel.id)}>Close</Btn>
                  </div>
                </div>
              )}
              {view==="dashboard"&&!inCase&&(
                <div style={{background:T.bg2,borderBottom:`1px solid ${T.border}`,padding:"0 16px",display:"flex",alignItems:"center",justifyContent:"space-between",height:50,flexShrink:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <div style={{width:7,height:7,borderRadius:"50%",background:T.emerald}}/>
                    <span style={{fontSize:10,color:T.textSub,fontFamily:"'JetBrains Mono',monospace",letterSpacing:"0.08em"}}>LEXAGENT · HALLUCINATION SHIELD ACTIVE</span>
                  </div>
                  <div style={{display:"flex",gap:6}}>
                    {isTablet&&<Btn variant="ghost" size="sm" icon="search" onClick={()=>setShowCmd(true)}>Search</Btn>}
                    <Btn variant="gold" size="sm" icon="plus" onClick={()=>setShowNew(true)}>New Matter</Btn>
                  </div>
                </div>
              )}
...
function CommandPaletteInner({onClose,onAction,cases}){
  ...
  const cmds = useMemo(()=>{
    const base = [
      ...
      {label:"Administration",desc:"Agent settings",icon:"admin",action:()=>onAction("admin")},
    ];
    const cc = cases.filter(c=>!q||c.title.toLowerCase().includes(q.toLowerCase())||c.client.toLowerCase().includes(q.toLowerCase())).map(c=>({
      label:c.title,desc:`${c.client} · ${c.caseType} · ${c.status}`,icon:"scale",action:()=>onAction("case",c)
    }));
    return [...(q?base.filter(b=>b.label.toLowerCase().includes(q.toLowerCase())):base),...cc].slice(0,9);
  },[q,cases]);
  ...
  const onKey = e=>{
    ...
    if(e.key==="Enter"&&cmds[sel]){cmds[sel].action();onClose();}
  };
      ...
      <div className="scroll-y" style={{maxHeight:"calc(60vh - 100px)"}}>
        {cmds.map((c,i)=>(
          <div key={i} onClick={()=>{c.action();onClose();}}
            style={{display:"flex",alignItems:"center",gap:10,padding:"10px 16px",cursor:"pointer",background:sel===i?T.panel2:"transparent",borderLeft:`2px solid ${sel===i?T.gold:"transparent"}`,transition:"all 0.1s"}}
            onMouseEnter={()=>setSel(i)}>
            <div style={{width:28,height:28,borderRadius:6,background:T.panel,border:`1px solid ${T.border}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
              <Icon n={c.icon} size={13} color={sel===i?T.gold:T.textSub}/>
            </div>
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:13,color:sel===i?T.text:T.platinum,fontWeight:sel===i?500:400}}>{c.label}</div>
              <div style={{fontSize:11,color:T.textMuted,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.desc}</div>
            </div>
            {sel===i&&<Icon n="arrow" size={12} color={T.gold}/>}
          </div>
        ))}
        {!cmds.length&&<div style={{padding:"24px",textAlign:"center",color:T.textMuted,fontSize:13}}>No results</div>}
      </div>
